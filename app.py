...
def on_boxClick(data):
    print(str(data['index']))
    print(str(data['playername']))
    print(str(data['turn']))
    currPlayers = [allUsers[0], allUsers[1]]
    data['currPlayers'] = currPlayers
...
def on_newUser(user):
    ...
    socketio.emit('newUser', {'allUsers':allUsers}, broadcast=True, include_self=False)
